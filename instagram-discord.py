...
INSTAGRAM_USERNAME = os.environ.get('popeleon22')
...
def webhook(webhook_url, html):
    ...
    embed["title"] = "Nueva publicación en @"+INSTAGRAM_USERNAME+""
    ...
    try:
        ...
        print("Imagen publicada con éxito, code {}.".format(
            result.status_code))
...
def main():
    try:
        ...
        if(os.environ.get("LAST_IMAGE_ID") == get_last_publication_url(html)):
            print("No hay nueva imagen para publicar en discord.")
        else:
            os.environ["LAST_IMAGE_ID"] = get_last_publication_url(html)
            print("Nueva imagen para publicar en discord.")
...
if __name__ == "__main__":
    if os.environ.get('popeleon22') != None and os.environ.get('WEBHOOK_URL') != None:
        while True:
            main()
            time.sleep(float(os.environ.get('TIME_INTERVAL') or 600)) # 600 = 10 minutes
    else:
        print('Configure las variables de entorno correctamente!')
